# Insight: Data Selection & Processing Strategy
**Date**: 2026-05-04
**Context**: User (Dev/Test) asked for better data selection and processing methods for the Bar Chart Race.
...
3. **Current Implementation**:
   - Hardcoded `RAW` data array in the HTML file.

## Recommended Strategy (Dev)
1. **Dynamic Loading**: Use `d3.csv` to load the file at runtime. This satisfies the "正确读取并处理数据" requirement.
2. **Aggregation**: Calculate annual means programmatically in JS.
...
2. **Edge Cases**: 
   ...
   - Ensure the animation doesn't break if a year is missing in the CSV.
