...
const POLICY_LABEL_MAX_WIDTH = 24;
const POLICY_LABEL_FONT_SIZE = 13.5;
const POLICY_LABEL_LINE_HEIGHT = 17;
const COLORS = {
  decrease: "#0072B2",
  neutral: "#F7F7F2",
  increase: "#D55E00",
...
function parseSummary(d) {
  return {
    policy: d.policy,
    startMonth: d.start_year,
    endYear: +d.end_year,
    pollutant: d.pollutant,
    baselineYear: +d.baseline_year,
    calculationStartMonth: d.calculation_start_month,
    ...
    maxChange: d.max_reduction_value === "" ? null : +d.max_reduction_value,
    status: d.status,
  };
}
...
function parseMonthly(d) {
  return {
    ...
    baselineMonthInfluence: +d.baseline_month_influence,
    change: +d.change_value,
    thresholdChange: +d.threshold_change_value,
    changePercent: d.change_pct === "" ? null : +d.change_pct,
...
function renderHeatmap() {
  ...
  const cellHeight = Math.max(
    52,
    maxPolicyLines * POLICY_LABEL_LINE_HEIGHT + 12
  );
  const margin = {
    top: 52,
    right: 8,
    bottom: 24,
    left: Math.ceil(maxPolicyLineWidth * POLICY_LABEL_FONT_SIZE) + 34,
  };
  const scrollHost = document.querySelector("#heatmap-scroll");
  const availableWidth = scrollHost?.clientWidth || 960;
  const minimumCellWidth = 88;
  const cellWidth = Math.max(
    minimumCellWidth,
    (availableWidth - margin.left - margin.right) / POLLUTANTS.length
  );
  const width = Math.max(
    availableWidth,
    margin.left + margin.right + POLLUTANTS.length * cellWidth
  );
  const height = margin.top + margin.bottom + policies.length * cellHeight;
  ...
  const maxDecrease = Math.abs(
    d3.min(summaryData, (d) => Math.min(d.endChange, 0))
  ) || 1;
  const maxIncrease = d3.max(
    summaryData,
    (d) => Math.max(d.endChange, 0)
  ) || 1;
  const color = d3
    .scaleLinear()
    .domain([-maxDecrease, 0, maxIncrease])
    .range([COLORS.decrease, COLORS.neutral, COLORS.increase])
    .clamp(true);

  renderColorLegend(color, maxDecrease, maxIncrease);
      ...
      lines.forEach((line, index) => {
        text
          ...
          .attr(
            "dy",
            index === 0
              ? `${-(lines.length - 1) * POLICY_LABEL_LINE_HEIGHT / 2}px`
  ...
  cells
    ...
    .classed("selected", (d) => cellKey(d.policy, d.pollutant) === selectedKey)
  ...
  cells
    ...
    .attr("fill", (d) => readableTextColor(color(d.endChange)))
    .text((d) => `${formatPercent(d.endChange)}%`);
}

function renderColorLegend(color, maxDecrease, maxIncrease) {
  const width = 190;
  const canvas = d3
    .select("#color-legend")
    .html("")
  ...
  d3.range(width).forEach((pixel) => {
    const value = pixel <= center
      ? -maxDecrease + (pixel / center) * maxDecrease
      : ((pixel - center) / center) * maxIncrease;
...
function renderDetailHeader(summary) {
  ...
  d3.select("#detail-subtitle").text(
    `${summary.startMonth}—${summary.endYear}-12 政策执行期；月度变化固定与 ${summary.baselineYear} 年同月比较。`
  );
  ...
  const maximum = summary.maxMonth
    ? `${formatMonth(summary.maxMonth)} / ${formatValue(Math.abs(summary.maxChange))}`
    : "无数据";

  d3.select("#selection-stats").html(`
    <div class="stat"><span>结束年较政策前基准期变化</span><strong>${formatPercent(summary.endChange)}%</strong></div>
    <div class="stat"><span>连续两月达标生效月份</span><strong>${effective}</strong></div>
    <div class="stat"><span>最大下降量</span><strong>${maximum}</strong></div>
  `);
}
...
function renderLineChart(data, summary, animate = false) {
  ...
  const gradient = defs
    ...
    .attr("y1", "0")
    .attr("y2", "1");
  gradient.append("stop").attr("offset", "0%").attr("stop-color", COLORS.decrease).attr("stop-opacity", 0.24);
  gradient.append("stop").attr("offset", "100%").attr("stop-color", COLORS.decrease).attr("stop-opacity", 0.02);
  ...
  const thresholdValue = (d) => d.thresholdChange;
  const values = data
    .map((d) => d.change)
    .concat(data.map(thresholdValue), [0]);
  const yExtent = d3.extent(values);
  const ySpan = yExtent[1] - yExtent[0];
  const yPadding = Math.max(ySpan * 0.08, 0.5);
  const yDomain = [
    yExtent[0] - yPadding,
    yExtent[1] + yPadding,
  ];
  ...
  const yAxis = d3.axisLeft(y).ticks(7).tickFormat(formatAxisValue);
  ...
  const thresholdLabel = svg
    .append("text")
    .attr("class", "annotation-label threshold-label")
    .attr("x", width - margin.right)
    .attr("y", y(thresholdValue(data[data.length - 1])) - 7)
    .attr("text-anchor", "end")
    .attr("fill", COLORS.increase)
    .attr("font-size", 14)
  ...
  const occupiedLabelBoxes = [
    ...curveObstacleBoxes,
    paddedBox(thresholdLabel.node().getBBox(), 9),
  ];
  const annotations = [
    {
  ...
  svg
    ...
    .on("mousemove", (event) => {
      ...
      showTooltip(event, `
        ...
        <div class="value">指数变化 ${formatValue(point.change)}</div>
        <div>变化率：${point.changePercent == null ? "无数据" : `${formatPercent(point.changePercent)}%`}</div>
        <div>本月情况：${formatValue(point.influence)}</div>
        <div>${summary.baselineYear} 年同月情况：${formatValue(point.baselineMonthInfluence)}</div>
      `);
    });

  thresholdLabel.raise();
}
...
function addAnnotation(
  ...
  const labelText = `${label} ${formatMonth(point.month)}`;
...
function readableTextColor(fill) {
  ...
  return perceivedBrightness < 150 ? "#fff" : "#26333e";
}
